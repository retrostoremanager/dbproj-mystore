-- Issue retrostoremanager/orchestrator-mystore#379
-- Add company_country column to company table and persist it via company_update_profile / company_get_profile.
-- Without this, PUT /company/profile silently drops the companyCountry field.

ALTER TABLE company ADD COLUMN IF NOT EXISTS company_country TEXT;

COMMENT ON COLUMN company.company_country IS 'Company country (free-text). Optional, used for profile display and reporting.';

-- Drop existing function signatures (required when signature changes)
DROP FUNCTION IF EXISTS company_get_profile(INTEGER);
DROP FUNCTION IF EXISTS company_update_profile(INTEGER, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT);
...
COMMENT ON FUNCTION company_get_profile(INTEGER) IS 'Returns company profile for display and edit (includes company_country).';
COMMENT ON FUNCTION company_update_profile(INTEGER, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT) IS 'Updates company profile fields (includes company_country).';
